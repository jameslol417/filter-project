...
class FastSAMPrompt:
    ...
    def plot_to_result1(self, img,
        ...
        if self.device == 'cpu':
            ...
            if isinstance(annotations[0], np.ndarray):
                annotations = torch.from_numpy(annotations)
            s_mask = self.fast_show_mask_gpu(
                annotations,
                plt.gca(),
                random_color=mask_random_color,
                bboxes=bboxes,
        ...
        if withContours:
            contour_all = []
            temp = np.zeros((original_h, original_w, 1))
            # temp = np.zeros((original_h, original_w),dtype=np.uint8)  # Use a grayscale image instead of a single-channel image
            mask_image = np.zeros_like(img)  # Create a blank mask of the same shape as the original image
            for i, mask in enumerate(annotations):
                if type(mask) == dict:
                    mask = mask['segmentation']
                annotation = mask.astype(np.uint8)
                ...
                for contour in contours:
                    contour_all.append(contour)
                cv2.drawContours(temp, contour_all, -1, (255, 255, 255), 2)
                color = np.array([30 / 255, 144 / 255, 255 / 255, 0.8]) # color of contour
                contour_mask = temp / 255 * color.reshape(1, 1, -1)
                plt.imshow(contour_mask)

            # mask_gray = cv2.cvtColor(s_mask, cv2.COLOR_BGR2GRAY)  # Convert the mask to grayscale
            # # Combine the original image and the grayscale mask
            # final_image = cv2.addWeighted(image, 0.5, cv2.cvtColor(s_mask, cv2.COLOR_RGBA2RGB), 0.5, 0, dtype=cv2.CV_8U)
            #
    ...
    def fast_show_mask(
        ...
        if random_color:
            ...
            color = np.ones((msak_sum, 1, 1, 3)) * np.array([0 / 255, 0 / 255, 0 / 255])
    ...
    def fast_show_mask_gpu(
        ...
        target_height=960,
        target_width=960,
        apply_gaussian_blur=True,  # Add this parameter to control whether to apply Gaussian blur
        gaussian_kernel_size=(5, 5),  # Adjust the kernel size as per your requirement
    ):
        msak_sum = annotation.shape[0]
        height = annotation.shape[1]
        weight = annotation.shape[2]
        ...
        if apply_gaussian_blur:
            # Convert 'show_cpu' to uint8 format before applying Gaussian blur
            show_cpu_uint8 = (show_cpu * 255).astype(np.uint8)

            # Apply Gaussian blur
            show_cpu_blurred = cv2.GaussianBlur(show_cpu_uint8, gaussian_kernel_size, 0)

            # Convert 'show_cpu_blurred' back to float32 format in the range [0, 1]
            show_cpu_blurred = show_cpu_blurred.astype(np.float32) / 255

            # Update 'show_cpu' with the blurred version
            show_cpu = show_cpu_blurred
        if not retinamask:
